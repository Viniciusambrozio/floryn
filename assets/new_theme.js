...
if ($(document).on("hide.bs.modal", "#related_popup_upsell", function() {
        where_to_go()
    }), eblstcky) {
    $("#mst-stiky-box").addClass("fix-search"), $(".stiky_button").on("click", function() {
        $("#AddToCartForm").find('button[type="submit"]').trigger("click")
    });
    var myFuncsticky = function() {
        document.body.clientWidth <= 749 && $(document).scroll(function() {
            $(this).scrollTop() > $("#mst-stiky-box").parentsUntil(".template-product").find("#shopify-section-footer").offset().top - 400 ? ($(".mst-stiky-div").fadeOut("fast"), $("#mst-stiky-box").removeClass("fix-search").hide(), ebl_pay_but && $(".shopify-payment-button").fadeOut("fast")) : ($(".mst-stiky-div").fadeIn("fast"), $("#mst-stiky-box").addClass("fix-search").show(), ebl_pay_but && $(".shopify-payment-button").fadeIn("fast"))
        })
    };
...
if (livestock ? setTimeout(function() {
    ...
    function pickasize() {
        if ("undefined" != typeof productOptions)
            for (i = 0; i < productOptions.length; i++) $(".single-option-selector:eq(" + i + ")").filter(function() {
                return $(this).find("option").length > 0
            }).prepend('<option value="">' + picka_text + "&nbsp;" + productOptions[i][i] + "</option>").val("").trigger("change")
    }
...
function swatch() {
    jQuery(".swatch :radio").change(function() {
        var e = jQuery(this).closest(".swatch").attr("data-option-index"),
            t = jQuery(this).val();
        console.log("Swatch changed - Option index:", e, "Value:", t);
...
$(function() {
    ...
    swatch();
    // Garantir que os swatches funcionem após a inicialização completa
    setTimeout(function() {
        swatch();
    }, 100);
}), $(document).on("shopify:section:select", function(e) {
    ...
    $("#shopify-section-" + e.detail.sectionId).hasClass("social-feeds-section") && $(".social-feeds-wrap").each(function(e, t) {
        var o = $(this).find(".js-instafeed");
        instagram.loadContent({
            el: o,
            clientID: o.data("client-id"),
    ...
    if (e.find(".currency-block").click(), e.find(".currencies li").on("click", function() {
        ...
        if ("" == t) jQuery.getJSON("https://geoip.nekudo.com/api", function(e) {
            if (e.country.code) {
                ...
                if (document.cookie = "_shopify_country_code=" + t, t) {
